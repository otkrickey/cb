...
- **storage**: SQLite CRUD + FTS5全文検索 + カーソルベースページネーション + 自動クリーンアップ + touch_entry + blob 外部化（rusqlite bundled-sqlcipher）。`Mutex<Option<Storage>>`シングルトン。SQLCipherによるAES-256ページレベル暗号化。タイムスタンプはミリ秒単位。51テスト
